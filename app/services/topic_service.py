...
클러스터 결과 파일(`data/cluster_result_mv_*.json`)에서 `topic == "기타"`로
분류된 클러스터의 phrase들을 LLM에 보내, 정해진 후보 토픽 중 가장 적절한
하나로 재분류한다. 원본은 건드리지 않고 `*_reclassified.json`으로 출력한다.
...
TOPIC_DEFINITIONS = {
    ...
    "스토리/전개": "이야기 내용 자체 — 사건/플롯/전개/결말/개연성/반전",
    "캐릭터": "인물 설정, 인물 매력, 관계성, 주조연 캐릭터",
    "연출": "이야기를 보여주는 방식 — 장면 구성/카메라/편집/컷/분위기 연출",
    "영상미": "미장센, 화면, 색감, 촬영, 비주얼, 스케일",
    "음향/OST": "음악, OST, 사운드, BGM, 효과음",
    "몰입감/재미": "몰입, 집중, 흡입력, 영화 전체의 종합적 재미/지루함",
    "공포/긴장감": "공포 요소, 긴장감, 오싹함, 서스펜스",
    "감정/여운": "감동, 여운, 먹먹함, 공감, 눈물 등 정서적 반응",
    "메시지/주제": "감독/영화가 전달하려는 주제 의식, 상징, 사회적 함의, 교훈",
    "설정/세계관": "작품의 설정, 세계관, 소재의 개연성·매력",
    "속도감/러닝타임": "템포, 호흡, 늘어짐, 러닝타임 길이",
    "CG/VFX": "CG, 시각효과, 특수효과의 퀄리티",
    "완성도": "작품 전반의 완성도, 퀄리티, 디테일",
...
- 후보 1~14번(연기~완성도) 중 명확히 들어맞는 것이 있으면 그것을 선택합니다.
...
def reclassify_file(client: OpenAI, input_path: str) -> Tuple[str, List[Tuple[str, str]]]:
    """클러스터 결과 파일 1개를 읽어 기타 클러스터를 재분류한다.
