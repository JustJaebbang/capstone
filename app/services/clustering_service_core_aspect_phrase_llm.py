...
# Lazy cached singleton. The service is instantiated once on first use and
# reused afterwards (same runtime cost as a module-level singleton, but without
# crashing import when OPENAI_API_KEY is unset). Mirrors the graceful-fallback
# pattern used by llm_service / topic_service.
_default_service: Optional[PhraseLLMCoreAspectClusteringService] = None
_service_init_failed = False


def _get_service() -> Optional[PhraseLLMCoreAspectClusteringService]:
    global _default_service, _service_init_failed

    if _default_service is None and not _service_init_failed:
        try:
            _default_service = PhraseLLMCoreAspectClusteringService()
        except Exception as e:
            _service_init_failed = True
            print(f"[Phrase LLM core aspect] service unavailable, falling back to input clusters: {e}")
...
def cluster_by_core_aspect_phrase_llm(
    ...
    if service is None:
        # No API key / init failure: keep the upstream (HDBSCAN) clusters as-is.
        return cluster_response

    return service.cluster(cluster_response)
